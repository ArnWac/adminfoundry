"""adminfoundry init scaffold sanity (PR-11 hotfix #1).

The scaffold previously wrote ``ADMINFOUNDRY_SECRET_KEY=change-me-in-production``
as the example value. That literal string is the one
``CoreAdminConfig.validate()`` actively rejects, so a newcomer running
``adminfoundry init`` then ``adminfoundry doctor`` got a confusing
...
def test_init_env_example_does_not_use_rejected_default(tmp_path):
    ...
    assert "ADMINFOUNDRY_SECRET_KEY=change-me-in-production" not in body
...
def test_init_env_example_prompts_for_real_secret(tmp_path):
    ...
    assert "ADMINFOUNDRY_SECRET_KEY=" in body
