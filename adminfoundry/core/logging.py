...
:func:`adminfoundry.security.sanitize.sanitize_payload` before logging.
"""
...
def configure_logging(config: CoreAdminConfig) -> None:
    ...
    for handler in list(root.handlers):
        if getattr(handler, "_adminfoundry_owned", False):
            root.removeHandler(handler)

    handler = logging.StreamHandler(sys.stderr)
    handler._adminfoundry_owned = True  # type: ignore[attr-defined]
    if config.log_json:
