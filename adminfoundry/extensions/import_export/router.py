"""Import, export, and bulk-action routes.

These endpoints are registered by ImportExportExtension.get_routers().
They require JobsExtension to also be active (provides Job model and job_service).
"""
...
from adminfoundry.extensions.import_export.schemas import (
    BulkActionRequest,
    ImportRequest,
    ImportResult,
)
...
async def export_model(
    ...
    Field-level policies are enforced — hidden fields are omitted.
    Creates a Job record for audit tracking (requires JobsExtension).
    """
    import csv
    import io
...
@router.post("/{model_name}/import", response_model=ImportResult)
